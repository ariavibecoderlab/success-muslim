...
**Backend — LIVE on Supabase** (project `qjrwgksdpeasjsqgdwbp`, region ap-southeast-1 / Singapore):

| Step | Status | Notes |
|------|--------|-------|
| Supabase project created | ✅ | `success-muslim` — $10/mo |
| Database migrations | ✅ | All 36 migrations applied — 61 tables, 211 RLS policies, 4 storage buckets |
| Edge functions deployed | ✅ | All 15 ACTIVE: api-ai, api-wearables, api-health, api-salah, api-quran, api-dhikr, api-family, api-profile, api-sunnah, api-wealth, api-misc, api-checkin, api-productivity, api-admin, jakim-proxy |
| Frontend `.env` wired | ✅ | Points at the new Supabase project URL + anon key |
| Security advisor | ✅ | Fixed `security_definer_view` on `wearable_connections_safe` (added `security_invoker`); remaining items are pre-existing public-bucket-listing WARNs |

**Frontend — config ready, deploy pending user action:**
`wrangler.toml`, `.env.example`, `npm run deploy` script, SPA `_redirects` all in place.

**Still needs the user (needs their accounts/keys):**
- Supabase secrets: `ANTHROPIC_API_KEY`, `STRAVA_CLIENT_ID/SECRET/REDIRECT_URI`
- Supabase Auth: add the live domain to Site URL + Redirect URLs
- Cloudflare Pages: create project, set the 5 `VITE_*` env vars, deploy
- Custom domain: move nameservers to Cloudflare

---

## 🤖 AI COACH + ⌚ WEARABLE SYNC (2026-05-15)

> Full design + setup steps in `docs/AI_AND_WEARABLES_ROADMAP.md`.

| Change | Status | Notes |
|--------|--------|-------|
| DB migration `ai_and_wearables.sql` | ✅ | `ai_insights`, `ai_conversations`, `ai_messages`, `wearable_connections` (+safe view), `wearable_activities`; RLS on all; `steps_logs.external_id` for dedup |
| `api-ai` edge function (Claude) | ✅ | Coaching insights, goal/habit recommendations, activity auto-analysis, Deen Q&A chat — reasons over a server-built SPICE snapshot |
| `api-wearables` edge function | ✅ | Strava OAuth + sync, on-device `ingest` for HealthKit/Health Connect, Garmin/Fitbit scaffolded |
| `health-bridge.ts` | ✅ | Wraps `capacitor-health` — HealthKit (iOS) + Health Connect (Android), graceful no-op on web |
| Hooks | ✅ | `useAIInsights`, `useAIChat`, `useHealthSync`, `useWearables` |
| AI Coach page `/ai-coach` | ✅ | Insights tab (3 generators + cards) + Companion chat tab |
| `AIInsightCard` on Dashboard | ✅ | Surfaces today's coaching insight or a generate CTA |
| `WearableConnections` in Settings | ✅ | Connect/sync/unlink Apple Health, Health Connect, Strava; Garmin/Fitbit shown "coming soon" |
| `capacitor-health` dependency added | ✅ | Run `npm install && npx cap sync`; add Info.plist / AndroidManifest usage strings |
| Type bug fix `QadaSolatTrack.tsx` | ✅ | `dayLog` typed as `Record<string, number>` — cleared 2 pre-existing `tsc` errors |
| **Verification** | ✅ | `tsc -p tsconfig.app.json --noEmit` passes clean (exit 0) |

**Needs server config before live:** `ANTHROPIC_API_KEY`, `STRAVA_CLIENT_ID/SECRET/REDIRECT_URI` as Supabase secrets; deploy both functions; `supabase db push`.
